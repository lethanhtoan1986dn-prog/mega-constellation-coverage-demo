...
# Celestrak requires a proper User-Agent; Python's default is blocked with 403.
...
class DigitalTwinConfig:
    ...
    # ── Ground station & beam coverage ──
    # Each ground station: (name, latitude_deg, longitude_deg)
    ground_stations: Tuple[Tuple[str, float, float], ...] = (
        ("London",   51.5074,  -0.1278),
        ("Shanghai", 31.2304, 121.4737),
    )
    gs_min_elevation_deg: float = 25.0       # minimum look-angle above horizon
    satellite_altitude_km: float = 550.0      # altitude used for beam-footprint radius
    coverage_circle_lift: float = 1.002       # slight lift to avoid z-fighting on Earth
    coverage_num_points: int = 48             # circle / cone discretisation
    show_coverage: bool = True                # toggle all beam-coverage visuals
...
# ───────────────────────────────────────────────────────────
# Ground station & beam-coverage geometry helpers
# ───────────────────────────────────────────────────────────

def lat_lon_to_ecef(lat_deg: float, lon_deg: float,
                    radius: float = 1.0) -> np.ndarray:
    """Latitude / longitude (degrees) → 3-D position on a sphere.

    ECEF convention used here:
      X axis — equator at prime meridian (0° lon)
...
def compute_coverage_angles(earth_radius_km: float,
                            satellite_altitude_km: float,
                            min_elevation_deg: float) -> Tuple[float, float]:
    """Return (cone_half_angle_rad, earth_central_angle_rad).

    The computation follows the law of sines applied to the triangle
    Earth‑centre — satellite — ground point.  *cone_half_angle* is the
    maximum off‑nadir angle the satellite can steer and still hit a ground
    user whose local elevation is ≥ *min_elevation_deg*.  *earth_central_angle*
    is the angular radius of the resulting footprint on the Earth sphere.
    """
    R = earth_radius_km
    r = R + satellite_altitude_km                 # orbital radius
    eps = math.radians(min_elevation_deg)

    # sin(cone) / R  =  sin(90° + eps) / r  =  cos(eps) / r
    cone_half = math.asin(R * math.cos(eps) / r)
    earth_central = math.pi / 2.0 - eps - cone_half
    return cone_half, earth_central


def _local_tangent_frame(nadir: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Build orthonormal *east* and *north* vectors in the plane
    perpendicular to *nadir*."""
    if abs(nadir[2]) < 0.99:
        arbitrary = np.array([0.0, 0.0, 1.0])
    else:
        arbitrary = np.array([1.0, 0.0, 0.0])
    east = np.cross(arbitrary, nadir)
    east /= np.linalg.norm(east)
    north = np.cross(nadir, east)
    return east, north


def build_single_coverage(sat_pos: np.ndarray,
                          earth_central_angle: float,
                          num_points: int = 48,
                          earth_radius: float = 1.0,
                          lift: float = 1.002) -> Tuple[np.ndarray, np.ndarray]:
    """Build coverage-circle and cone-beam line geometry for **one** satellite.

    Returns
    -------
    circle_segs : (2 * num_points, 3)  – line‑segment vertices for the circle
    cone_segs   : (2 * num_points, 3)  – line‑segment vertices for the cone
    """
    sat_norm = np.linalg.norm(sat_pos)
    nadir = -sat_pos / sat_norm                     # satellite → Earth centre
    subsat = sat_pos / sat_norm * earth_radius      # nadir point on surface

    east, north = _local_tangent_frame(nadir)

    angles = np.linspace(0.0, 2.0 * math.pi, num_points, endpoint=False)
    cos_ang = np.cos(angles); sin_ang = np.sin(angles)
    cos_alpha = math.cos(earth_central_angle)
    sin_alpha = math.sin(earth_central_angle)

    # (num_points, 3) – points on the coverage circle
    directions = np.outer(cos_ang, north) + np.outer(sin_ang, east)
    circle_pts = cos_alpha * subsat + sin_alpha * directions * earth_radius

    # Build line segments for the closed circle
    n = num_points
    circle_segs = np.empty((2 * n, 3), dtype=np.float32)
    circle_segs[0::2] = circle_pts * lift
    circle_segs[1::2] = np.roll(circle_pts, -1, axis=0) * lift

    # Build line segments for the cone (satellite → each circle point)
    cone_segs = np.empty((2 * n, 3), dtype=np.float32)
    cone_segs[0::2] = sat_pos
    cone_segs[1::2] = circle_pts * lift

    return circle_segs, cone_segs


def check_sat_covers_gs(sat_pos: np.ndarray,
                        gs_pos: np.ndarray,
                        earth_central_angle: float,
                        earth_radius: float = 1.0) -> bool:
    """Return ``True`` when the satellite's footprint contains the ground station."""
    sat_norm = np.linalg.norm(sat_pos)
    subsat = sat_pos / sat_norm * earth_radius

    cos_dist = np.dot(subsat, gs_pos) / (earth_radius * earth_radius)
...
def load_starlink_data(url: str, reload: bool = True, max_satellites: int = None) -> Tuple[object, list, SatrecArray]:
    """
    ...
    Parameters:
        ...
        max_satellites (int): Maximum number of satellites to load (None = all).
    ...
    if max_satellites is not None:
        valid_satellites = valid_satellites[:max_satellites]
...
def setup_visualization(config: DigitalTwinConfig = DEFAULT_CONFIG) -> Dict[str, object]:
    ...
    view.add(coverage_circles)

    # Cone beams: thin lines from satellite to coverage-circle edge
    cone_beams = scene.visuals.Line()
    view.add(cone_beams)

    # Ground-station → satellite connection lines (bright blue, on/off)
...
class DigitalTwin:
    ...
    def __init__(
        ...
        # ── Ground stations (Earth-fixed ECEF positions) ──
        self.gs_names = [gs[0] for gs in config.ground_stations]
        self.gs_ecef = np.array([
            lat_lon_to_ecef(gs[1], gs[2], radius=1.0)
            for gs in config.ground_stations
        ])
        self.gs_color = np.array([1.0, 0.15, 0.15, 1.0])   # red marker

        # ── Beam-coverage geometry parameters ──
        cone_half, earth_central = compute_coverage_angles(
            config.earth_radius_km,
            config.satellite_altitude_km,
            config.gs_min_elevation_deg,
        )
        self.cone_half_angle = cone_half
        self.earth_central_angle = earth_central
        self.coverage_num_points = config.coverage_num_points
        self.coverage_lift = config.coverage_circle_lift

        # Pre-allocate colour arrays used every frame
        self._circle_color = np.array(
            [0.50, 0.72, 1.0, 0.45], dtype=np.float32)   # light blue
        self._cone_color = np.array(
            [0.55, 0.78, 1.0, 0.18], dtype=np.float32)   # lighter / more transparent
    ...
    # ── Beam-coverage update ──────────────────────────────────────
    def _update_coverages(self):
        """Update ground-station positions, every satellite's coverage circle,
        cone beam, and satellite↔ground-station connection lines."""
        if not self.config.show_coverage:
            return

        tic = time.perf_counter()

        # ── 1.  Rotate ground-station positions into the current ICRS frame ──
        rot_deg = self.compute_rotation()
        rot_rad = math.radians(rot_deg)
        cos_r = math.cos(rot_rad)
        sin_r = math.sin(rot_rad)

        n_gs = self.gs_ecef.shape[0]
        gs_pos = np.empty_like(self.gs_ecef)
        for j in range(n_gs):
            x, y, z = self.gs_ecef[j]
            gs_pos[j, 0] = x * cos_r - y * sin_r
        ...
        n_pts = self.coverage_num_points
        total_segs = n_sats * 2 * n_pts   # each segment = 2 vertices

        all_circles = np.empty((total_segs, 3), dtype=np.float32)
        all_cones = np.empty((total_segs, 3), dtype=np.float32)

        for i in range(n_sats):
            ci, co = build_single_coverage(
                self.positions[i],
                self.earth_central_angle,
                num_points=n_pts,
                lift=self.coverage_lift,
            )
            base = i * 2 * n_pts
            all_circles[base : base + 2 * n_pts] = ci
            all_cones[  base : base + 2 * n_pts] = co

        # Apply per-visual colours (Vispy broadcasts single-colour arrays)
        self.viz['coverage_circles'].set_data(
            pos=all_circles, color=self._circle_color,
            width=1.2, connect='segments',
        )
        self.viz['cone_beams'].set_data(
            pos=all_cones, color=self._cone_color,
            width=0.6, connect='segments',
        )

        # ── 3.  Satellite ↔ ground-station connections ─────────────
        conn_list = []
        for i in range(n_sats):
            sat_pos = self.positions[i]
            for j in range(n_gs):
                if check_sat_covers_gs(sat_pos, gs_pos[j],
                                       self.earth_central_angle):
                    conn_list.append(sat_pos)
                    conn_list.append(gs_pos[j] * 1.001)  # slight lift

        if conn_list:
            conn_data = np.array(conn_list, dtype=np.float32)
            self.viz['gs_connections'].set_data(
                pos=conn_data, color=self._conn_color,
                width=2.5, connect='segments',
            )
        else:
            self.viz['gs_connections'].set_data(pos=np.zeros((0, 3), dtype=np.float32))

        toc = time.perf_counter()
        self.profiled_time['coverages'] = toc - tic
...
def main():
    # Load Starlink data.
    # Use local TLE file if network is unavailable; otherwise fetch from Celestrak.
    satellite_url = os.path.join(os.path.dirname(__file__), 'gp.php')
    ...
    ts, valid_satellites, sat_array = load_starlink_data(satellite_url, reload=False, max_satellites=300)
